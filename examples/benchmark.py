    ...
    def benchmark_memory(self, concurrency: int, loop: int):
        for j in range(0, loop):
            ...
            for i in range(0, concurrency):
                futures_add.append(
                    memory_list[i].add(
                        Message("human", "Indexify is amazing!"),
                        Message(
                            "assistant", "How are you planning on using Indexify?!"
                        ),
                    )
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-u", "--url", default=DEFAULT_SERVICE_URL, help="indexify url to connect to"
    )
    parser.add_argument(
        "-c",
        "--concurrency",
    ...
    print(
        f"looping for {args.loop}, with concurrency is set to be: {args.concurrency}, "
