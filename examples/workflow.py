...
def pdf_extraction(content: Content) -> List[Content]:
    return [
        Content.from_text("this is stupid"),
        Content.from_text("fuck why"),
        Content.from_text("this isn't stupid"),
    ]
...
def text_chunks(content: Content, params: ChunkOptions) -> List[Content]:
    ...
    ret = [
        Content.from_text(i + "--" + j, features=features)
        for i, j in zip(text.split()[:-1], text.split()[1:])
    ]
