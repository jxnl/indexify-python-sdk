...
class TestIntegrationTest(unittest.TestCase):
    ...
    def test_add_documents(self):
        ...
        client.add_documents(
            "test_graph",
            Document(text="This is a test", labels={"source": "test"}, id=None),
        )
        ...
        client.add_documents(
            "test_graph",
            [
                Document(text="This is a new test", labels={"source": "test"}, id=None),
                Document(
        ...
        client.add_documents(
            "test_graph",
            ["string", Document("document string", {}, id=None)],
            doc_id=None,
        )
    ...
    def test_get_content(self):
        ...
        content = client.get_extracted_content(
            ingested_content_id="idontexist",
            graph_name="test_get_content_graph",
            policy_name="content_policy",
        )
    ...
    def test_download_content(self):
        ...
        content_id = client.add_documents(
            "test_download_graph", ["test download"], doc_id=None
        )
        client.wait_for_extraction(content_id)
        ...
        assert "test download" in data.decode(
            "utf-8"
        )  # chunk may not be the whole string
    ...
    def test_search(self):
        ...
        results = client.search_index(
            f"{extractor_name}_graph.{extractor_name}.embedding", "LLM", 1
        )
    ...
    def test_get_extraction_graphs(self):
        ...
        assert (
            len(list(filter(lambda x: x.name.startswith(name), extraction_graphs))) == 1
        )
    ...
    def test_ingest_remote_url(self):
        ...
        res = self.client.ingest_remote_file(
            "default", url, "image/gif", {}, id=content_id
        )
