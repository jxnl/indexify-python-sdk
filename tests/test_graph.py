...
def extractor_b(file: File) -> List[FileChunk]:
    return [
        FileChunk(data=file.data, start=0, end=len(5)),
...
class TestGraphA(unittest.TestCase):
    def test_run_graph(self):
        ...
        self.assertEqual(
            results,
            {
                "extractor_a": [File(data=b"hello", mime_type="text/plain")],
                "extractor_b": [
                    FileChunk(data=b"hello", start=0, end=5),
                    FileChunk(data=b"hello", start=5, end=5),
                ],
                "extractor_c": [SomeMetadata(metadata={"a": "b", "c": "d"})],
            },
        )
